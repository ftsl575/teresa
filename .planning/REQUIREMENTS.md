...
- [x] **WF-02**: `CONTRIBUTING.md` exists at the repo root documenting the GSD development cycle (`/gsd-discuss-phase` → `/gsd-plan-phase` → `/gsd-execute-phase` → `/gsd-verify-work`), how to run tests, how to add a vendor (pointer to `spec_classifier/docs/dev/NEW_VENDOR_GUIDE.md`), and the project's "do not fix" tech-debt rules.
