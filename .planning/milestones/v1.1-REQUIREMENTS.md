# Requirements Archive: v1.1 Periphery cleanup (residual)
...
- [x] **CACHE-01**: `run.ps1` reads `temp_root` from `spec_classifier/config.local.yaml` and sets `$env:PYTHONPYCACHEPREFIX = Join-Path $temp_root "__pycache__"` before any Python invocation.
- [x] **CACHE-02**: `teresa_gui.py` sets `os.environ["PYTHONPYCACHEPREFIX"]` from the same `temp_root` source in `__main__`, before any `subprocess.Popen`/`subprocess.run` invocation.
- [x] **CACHE-03**: `run.ps1` invokes `clean.ps1` by default and accepts a `-NoClean` switch to opt out.
- [x] **CACHE-04**: `docs/dev/ONE_BUTTON_RUN.md` "Workspace cleanup" section reflects clean-by-default + `-NoClean` opt-out.
