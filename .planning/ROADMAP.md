# Roadmap: Teresa — Cleanup & Workflow Setup Milestone
...
**Success Criteria** (what must be TRUE):
  ...
  4. `pytest tests/ -v --tb=short` (run from `spec_classifier/`) still passes with the same skip ratio behavior — the cleanup must not trip `conftest.py:pytest_sessionfinish` (`skipped/total > 0.50` or `passed == 0`).
...
- Launchers stay Windows-only by design — only the username string is scrubbed; `run.ps1`, `teresa.bat`, `teresa_gui.py` remain PowerShell/Windows.
**Plans**: TBD
...
**Success Criteria** (what must be TRUE):
  ...
  3. Root `CLAUDE.md` is a thin pointer/repo-layout note — no business rule, E-code table, alias-table, or pipeline-stage detail is duplicated from `spec_classifier/CLAUDE.md`; the deep reference remains in `spec_classifier/CLAUDE.md`.
...
**Constraints honored**:
- No business-rule re-derivation — copy/preserve the existing wording in `spec_classifier/CLAUDE.md` (`power_cord=None`, LOGISTIC scope, BASE rules, alias-table semantics, "do not fix" markers all stay verbatim where they live today).
...
**Success Criteria** (what must be TRUE):
  ...
  2. `CONTRIBUTING.md` exists at the repo root and documents (a) the GSD cycle by command, (b) how to run pytest from `spec_classifier/` including the skip-ratio gate, (c) how to add a vendor (link only — points at `spec_classifier/docs/dev/NEW_VENDOR_GUIDE.md`, no duplication), and (d) the project's "do not fix" tech-debt rules verbatim from PROJECT.md and `.planning/codebase/CONCERNS.md` BLOCKER section (`power_cord=None`, `core/parser.py` Dell-specific, `batch_audit.py` reads Excel, YAML rule order load-bearing, `HW_TYPE_VOCAB` duplication).
...
**Constraints honored**:
- `CONTRIBUTING.md` documents the existing GSD cycle and the project's existing "do not fix" rules — it does not re-author rules or invent new policy.
