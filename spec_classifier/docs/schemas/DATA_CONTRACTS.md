...
- Lenovo: server, cpu, ram, gpu, storage_ssd, storage_hdd, storage_nvme, raid_controller, hba, nic, network_adapter, transceiver, sfp_cable, psu, power_cord, fan, heatsink, riser, chassis, motherboard, backplane, rail, blank_filler, management, tpm, bezel, accessory
- xFusion: server, cpu, memory, storage_ssd, storage_hdd, storage_nvme, raid_controller, hba, nic, transceiver, sfp_cable, psu, power_cord, fan, riser, backplane, rail, enablement_kit
...
**hw_type** (26 значений): server, switch, storage_system, wireless_ap, cpu, memory, gpu, storage_drive, storage_enclosure, storage_controller, hba, backplane, io_module, network_adapter, transceiver, cable, psu, fan, heatsink, riser, chassis, rail, blank_filler, management, tpm, accessory.
...
**cleaned_spec.xlsx** — фильтрованная спецификация: только ITEM-строки с entity_type из config cleaned_spec.include_types и (опционально) state PRESENT. Колонки: Group Name, Group ID, Module Name, Option Name, SKUs, Qty, Option ID, Unit Price, Device Type, HW Type, Entity Type, State, Source Row, Rule ID.

| Колонка | Тип | Описание |
|---------|-----|----------|
| Group Name … State | (см. выше) | Существующие колонки — порядок неизменен. |
| `Source Row` | int | `source_row_index` из NormalizedRow: 1-based номер строки в исходном Excel. |
| `Rule ID` | str | `matched_rule_id` из ClassificationResult: идентификатор сработавшего правила (пустая строка если None). |
...
| `sku_examples` | str | SKU (part number / Product #) из первых 3 кандидатов кластера, разделитель ` | `. |
| `module_examples` | str | module_name / config_name из первых 3 кандидатов кластера, разделитель ` | `. |
...
Генерируется `batch_audit.py --output-dir <dir>`. Файл создаётся в корне `<dir>`.

- **Формат:** xlsx, лист «Сводный отчёт»; одна строка — одна проблемная строка из аудируемого файла.
- **Назначение:** агрегированный отчёт по всем вендорам / файлам для ручной ревизии. Строки с `pipeline_check = "OK"` не включаются.

| Колонка | Тип | Обязательность | Описание |
|---------|-----|----------------|----------|
| `Документ` | str | да | Имя исходного файла (`*_annotated.xlsx`). |
| `Вендор` | str | да | Вендор: `dell`, `cisco`, `hpe`, или `unknown`. |
| `Артикул / SKU` | str | нет | SKU / Part Number строки. Пустая строка если отсутствует. |
| `Описание` | str | да | `option_name` строки. |
| `Module Name` | str | нет | `module_name` / `config_name`. Пустая строка если отсутствует. |
| `Entity Type` | str | нет | Классифицированный тип: BASE, HW, CONFIG, … |
| `HW Type` | str | нет | `hw_type` строки. Пустая строка если null. |
| `Device Type` | str | нет | `device_type` строки. Пустая строка если null. |
| `State` | str | нет | PRESENT / ABSENT / DISABLED. |
| `Пометка скрипта` | str | да | `pipeline_check`-значение: E-код(ы) через `;`, `AI_MISMATCH:…`, `AI_SUGGEST:…`. |
| `Комментарий` | str | нет | Всегда пустая строка при генерации (поле для ручных аннотаций). |

**Цветовая кодировка строк** (заливка ячеек):

| Пометка | Цвет |
|---------|------|
| `AI_MISMATCH` | светло-голубой (`DDEEFF`) |
| `MANUAL_CHECK` | светло-оранжевый (`FFE0B2`) |
| `AI_SUGGEST` | светло-зелёный (`E8F5E9`) |
| `E2` | светло-красный (`FFCCCC`) |
| `E13` / `E14` / `E9` | персиковый (`FFE0CC`) |
| `E15` / `E17` | серый (`F5F5F5` / `FAFAFA`) |
| прочие | белый (`FFFFFF`) |

**Примечание:** строки сортируются по файлу (порядок обхода output_dir); внутри файла — порядок из `*_annotated_audited.xlsx`. Строки без проблем (`pipeline_check = "OK"`) в файл не попадают.
...
| `hw_type` | `hw_type` | Нормализованный аппаратный тип из HW_TYPE_VOCAB (26 значений). Только для HW-строк. |
| `row_kind` | `row_kind` | Тип строки: `"ITEM"` (данные) или `"HEADER"` (заголовок / разделитель). |
| `source_row_index` | `Source Row` | 1-based номер строки в исходном Excel. Колонка `"Source Row"` в cleaned_spec.xlsx. |
| `matched_rule_id` | `Rule ID`, `rule_id` | Идентификатор сработавшего правила; пустая строка в cleaned_spec.xlsx если None. |
