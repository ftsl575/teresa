...
**Примеры `device_type` из таксономических циклов (не заменяют YAML):** cycle 1 — `motherboard`, `storage_enclosure`, `backplane`, `bezel` (разные вендоры); cycle 2 — без новых литералов `hw_type`; добавлены/уточнены метки `device_type`: `front_panel`, `power_distribution_board`, `interconnect_board`, `media_bay`, `air_duct`, `optical_drive`, плюс унификация Lenovo `drive_cage`/`battery` там, где они появились в правилах. См. `docs/taxonomy/hw_type_taxonomy.md` (PR-11 master map).
