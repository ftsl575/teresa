...
## Версия: v4.1 (QA patch — точечные исправления v4)
...
**НЕ менять:** `ClassificationResult`, `NormalizedRow`, `src/core/*`, порядок и логику других функций в json_writer.py.
...
```python
# Заменить функцию _classification_result_to_dict:
def _classification_result_to_dict(result: ClassificationResult, source_row_index: int) -> dict:
...
# Заменить функцию save_classification:
def save_classification(
    ...
    with open(path, "w", encoding="utf-8") as f:
        for result, nrow in zip(results, normalized_rows):
            line = json.dumps(
                _classification_result_to_dict(result, nrow.source_row_index),
                ensure_ascii=False,
            ) + "\n"
            f.write(line)
```
...
# source_row_index присутствует и является int
python -c "
import json
lines = open(r'$($run.FullName)\classification.jsonl').readlines()
for i, line in enumerate(lines):
    obj = json.loads(line)
    assert 'source_row_index' in obj, f'Missing at line {i}'
    assert isinstance(obj['source_row_index'], int), f'Not int at line {i}'
print(f'OK: {len(lines)} lines, all have source_row_index')
"

# Число строк в classification.jsonl равно числу строк в rows_normalized.json
python -c "
import json
