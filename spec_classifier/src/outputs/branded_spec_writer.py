...
PREAMBLE_SECTION_COLOR = "FFE8B3"
PREAMBLE_SECTION_LABEL = "Позиции без привязки к серверу"
...
def _write_table_header(ws, row: int) -> int:
    headers = {2: "Артикул", 3: "Наименование", 4: "Кол-во", 5: "Цена"}
...
def _write_base_row(ws, row: int, option_name: str, skus: List[str], qty, option_price) -> int:
    ...
    ws.cell(row=row, column=5, value=option_price).fill = _fill(C_PRIMARY)
...
def _write_data_row(ws, row: int, skus: List[str], option_name: str, qty, option_price, zebra: bool) -> int:
    ...
    ws.cell(row=row, column=5, value=option_price).fill = _fill(bg)
    ws.cell(row=row, column=5).number_format = "0.00"
    for col in (2, 3, 4, 5):
        c = ws.cell(row=row, column=col)
...
def generate_branded_spec(
    ...
    for nrow, result in zip(normalized_rows, classification_results):
        ...
        if result.entity_type == EntityType.BASE:
            ...
            continue
        if current is None:
            preamble_rows.append((nrow, result))
            dropped_preamble_count += 1
            continue
    ...
    ws.column_dimensions["E"].width = COL_E_WIDTH
    ...
    if preamble_rows:
        for col in (2, 3, 4, 5):
            c = ws.cell(row=current_row, column=col,
                        value=PREAMBLE_SECTION_LABEL if col == 3 else None)
            c.fill = _fill(PREAMBLE_SECTION_COLOR)
            c.font = _font(bold=True, size=10, italic=True)
            c.alignment = Alignment(horizontal="left", vertical="center")
        ws.row_dimensions[current_row].height = 16
        current_row += 1
        for zebra_idx, (nrow, _) in enumerate(preamble_rows):
            current_row = _write_data_row(
                ws, current_row,
                skus=list(nrow.skus),
                option_name=nrow.option_name,
                qty=nrow.qty,
                option_price=nrow.option_price,
                zebra=(zebra_idx % 2 == 1),
            )
        ws.row_dimensions[current_row].height = 8
        current_row += 1

    if dropped_preamble_count:
        logging.warning(
            "branded_spec: found %d ITEM rows before first BASE; rendered in preamble block",
            dropped_preamble_count,
        )
    ...
    for block in blocks:
        ...
        current_row = _write_base_row(
            ...
            qty=base_nrow.qty,
            option_price=base_nrow.option_price,
        )
